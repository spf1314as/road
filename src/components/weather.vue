<template>
  <div class="one-container">
    <div>
       <!-- <button class="btn" id="idweather" @click="go('')">天气状况</button> -->
       <button class="btn" id="idtraffic" @click="go('t')">流量统计</button>
       <button class="btn" id="idmonitor" @click="go('v')">实时监控</button>
    </div>
    <div class="above">
      <!-- 左侧gif -->
...
export default {
  ...
  methods: {
    ...
    go (flag) {
      this.$router.push({name: flag === 'v' ? 'video' : 'traffic'})
    }
  },
  mounted () {
    // this.getData()
